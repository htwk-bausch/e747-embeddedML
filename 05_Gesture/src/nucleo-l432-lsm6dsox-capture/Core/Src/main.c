...
int main(void)
{
  ...
  const float accThreshold = 2500.0; // in mG
  ...
  while (1)
  {
	...
	while (samplesRead < numSamples) {
      ...
      if (dataRdy) {
		...
		// convert acc axes to mG
		printf("%.1f,%.1f,%.1f,", ((float_t)acc_axes.x * LSM6DSOX_ACC_SENSITIVITY_FS_4G),
		                          ((float_t)acc_axes.y * LSM6DSOX_ACC_SENSITIVITY_FS_4G),
							      ((float_t)acc_axes.z * LSM6DSOX_ACC_SENSITIVITY_FS_4G));
...
static void MEMS_Init(void) {
  ...
  LSM6DSOX_ACC_SetFullScale(&MotionSensor, 4);             // [-4G; +4G]
  ...
  LSM6DSOX_GYRO_Enable(&MotionSensor);                     // enable gyroscope
}
